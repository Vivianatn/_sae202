...
function getutilisateursAlphabetiques()
{
    ...
    try {
        ...
        $req = $db->query('SELECT * FROM utilisateurs ORDER BY client_prenom ASC');
...
function getQuatreutilisateurs()
{
    ...
    try {
        ...
        $req = $db->query('SELECT * FROM utilisateurs ORDER BY client_code DESC LIMIT 4');
...
{
    $db = new PDO('mysql:host='.HOST.';dbname='.DBNAME.'', USER, PASSWORD);
    $req = $db->prepare('SELECT * FROM utilisateurs WHERE user_email == '.$utilisateur_mail.'');
...
{
    $db = new PDO('mysql:host='.HOST.';dbname='.DBNAME.'', USER, PASSWORD);
    $req = $db->prepare('INSERT INTO utilisateurs (user_prenom, user_nom, user_email, user_tel, user_mdp) 
                           VALUES (:user_prenom, :user_nom, :user_email, :user_tel, :usermdp)');
    $req->execute([
